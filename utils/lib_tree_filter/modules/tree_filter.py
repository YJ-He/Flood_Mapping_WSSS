...
class TreeFilter2D(nn.Module):
    '''
    Tree Filtering(project and refine)
    '''
    ...
    def build_edge_weight(self, fm, sorted_index, sorted_parent, low_tree, filenames=None):
        batch  = fm.shape[0]
        channel = fm.shape[1]
    ...
    def forward(self, feature_in, embed_in, tree, low_tree=True, filenames=None):
        ori_shape = feature_in.shape
        sorted_index, sorted_parent, sorted_child = bfs(tree, 4) # breadth-first sorting (BFS) algorithm
        edge_weight = self.build_edge_weight(embed_in, sorted_index, sorted_parent, low_tree,filenames)
